...
# profile_kernel_nsys.py so ONLY the measured region is traced (model load
# and warmup are excluded -> small, readable trace).
#
# Output: profiling/profile-kernels/nsys-profiler/out/<kernel>_nsys.nsys-rep  (open in the Nsight Systems GUI).
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
OUT=profiling/profile-kernels/nsys-profiler/out/${KERNEL:-attention}_nsys
nsys profile \
  ...
  --output="$OUT" \
  uv run python profiling/profile-kernels/nsys-profiler/profile_kernel_nsys.py "${KERNEL:-attention}"
